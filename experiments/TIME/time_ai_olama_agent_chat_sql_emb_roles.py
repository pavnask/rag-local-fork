...
# Ensure chat_memory table has required columns
...
def retrieve_memory(limit=5):
    cursor.execute(
        "SELECT id, user_query, ai_response, role, feedback FROM chat_memory ORDER BY timestamp DESC LIMIT ?", (limit,))
...
def chatbot():
    console = Console()
    console.print(
        "\n🤖 [bold cyan]IT Systems Self-Learning Chatbot Ready![/bold cyan] Type your question or 'exit' to quit.\n",
        style="bold green")
    ...
    while True:
        ...
        # Process query using multiple AI agents
        ...
        for role, (role_name, response) in agent_responses.items():
            ...
            save_memory(user_input, response, role_name)
