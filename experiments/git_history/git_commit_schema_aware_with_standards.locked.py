...
def check_against_standards(parsed_yaml, standards):
    ...
    def get_systems_block(yaml_data):
        ...
        # Step 1: Try direct access to kadzo.v2023.systems
        direct_block = yaml_data.get("kadzo.v2023.systems")
        if isinstance(direct_block, dict):
            print(Fore.YELLOW + f"[DEBUG] kadzo.v2023.systems keys: {list(direct_block.keys())}")
            return direct_block

        # Step 2: Look for any flat or semi-nested keys
        flat_systems = {}

        for k, v in yaml_data.items():
            if isinstance(v, dict) and '.' in k and 'systems' in k:
                flat_systems[k] = v
            elif isinstance(v, dict):
                for sub_k, sub_v in v.items():
                    if isinstance(sub_v, dict) and '.' in sub_k and 'systems' in sub_k:
                        flat_systems[sub_k] = sub_v

        if flat_systems:
            print(Fore.CYAN + f"[DEBUG] Reconstructed systems block with {len(flat_systems)} entries.")
            print(Fore.MAGENTA + f"[DEBUG] flat_systems keys: {list(flat_systems.keys())}")
            return flat_systems

        # Step 3: Fallback — first nested dict-of-dicts
    ...
    print(Fore.YELLOW + f"[DEBUG] parsed_yaml from file: {parsed_yaml}")
    systems_block = get_systems_block(parsed_yaml)
    print(Fore.YELLOW + f"[DEBUG] Confirmed systems_block type: {type(systems_block)} with keys: {list(systems_block.keys())}")
    print(Fore.CYAN + f"[DEBUG] Systems block keys: {list(systems_block.keys())}")
    ...
    print(Fore.MAGENTA + f"[DEBUG] Final systems_block content: {systems_block}")
    print(Fore.YELLOW + "[DEBUG] Starting loop over systems_block...")

    for sys_id, sys_data in systems_block.items():
        if sys_id in {"title", "description", "class", "group", "criticality"}:
            print(Fore.LIGHTBLACK_EX + f"[SKIP] {sys_id} is a known metadata field.")
            continue

        if not isinstance(sys_data, dict):
            print(Fore.LIGHTBLACK_EX + f"[SKIP] {sys_id} is not a dict, got {type(sys_data).__name__}")
            continue

        if "criticality" not in sys_data:
            print(Fore.YELLOW + f"[DEBUG] {sys_id} missing 'criticality'")
        if "group" not in sys_data:
            print(Fore.YELLOW + f"[DEBUG] {sys_id} missing 'group'")

        if "criticality" not in sys_data or "group" not in sys_data:
            print(Fore.CYAN + f"[DEBUG] {sys_id} -> criticality: {sys_data.get('criticality')}, group: {sys_data.get('group')}")
            print(Fore.YELLOW + f"[DEBUG] {sys_id} skipped due to missing required fields.")
            continue

        print(Fore.CYAN + f"[DEBUG] Evaluating {sys_id} against standards...")
        for item in standards:
            levels = item.get("sber", {}).get("criticality", [])
            required_obj = item.get("sber", {}).get("obj_und_ctrl", [])
            print(Fore.MAGENTA + f"[DEBUG] Standard levels: {levels}")
            print(Fore.MAGENTA + f"[DEBUG] Standard obj_und_ctrl: {required_obj}")
            print(Fore.BLUE + f"[DEBUG] Comparing system: {sys_id}")
            print(Fore.BLUE + f"  - System criticality: {sys_data['criticality']} vs Levels: {levels}")
            print(Fore.BLUE + f"  - System group: {sys_data['group']} vs Required groups: {required_obj}")
            normalized_crit = str(sys_data["criticality"]).strip().lower()
            normalized_group = str(sys_data["group"]).strip().lower()
            print(Fore.BLUE + f"  - Normalized criticality: {normalized_crit} vs Levels: {[lvl.strip().lower() for lvl in levels]}")
            print(Fore.BLUE + f"  - Normalized group: {normalized_group} vs Groups: {[grp.strip().lower() for grp in required_obj]}")
            if (
                normalized_crit in [lvl.strip().lower() for lvl in levels] and
                normalized_group in [grp.strip().lower() for grp in required_obj]
            ):
...
def main():
    ...
    requirements = {}
    standards = []
    print(Fore.YELLOW + f"[DEBUG] args_dict['standards_file'] = {args_dict.get('standards_file')}")
    if args_dict.get("standards_file"):
        standards_path = os.path.abspath(args_dict["standards_file"])
        standards = load_standards(standards_path)
        print(Fore.CYAN + f"[DEBUG] Loaded {len(standards)} standards from {standards_path}")
